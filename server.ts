...
async function startServer() {
  ...
  let posts: any[] = [
    ...
    },
    {
      ...
      type: 'VIBE',
      isUncensored: false,
      tags: ['decentralization', 'validators', 'infrastructure'],
      intensityScore: 60,
  ...
  app.get('/api/search', (req, res) => {
    ...
    if (!q && !tag) return res.json({ users: [], posts: [] });
    ...
    if (q) {
      ...
      postResults = postResults.filter(p => 
    ...
    if (tag) {
      ...
      postResults = postResults.filter(p => 
        p.tags && p.tags.some((t: string) => t.toLowerCase() === tagQuery)
      );
    }

    res.json({ users: q ? userResults : [], posts: postResults });
  });
  ...
  app.put('/api/users/me', (req, res) => {
    const { username, professional_bio, is_verified, has_completed_onboarding } = req.body;
    ...
    if (has_completed_onboarding !== undefined) currentUser.has_completed_onboarding = has_completed_onboarding;
    res.json(currentUser);
  });
  ...
  app.get('/api/feed', (req, res) => {
    ...
    // Sophisticated Feed Algorithm
    // Score = (Engagement * 2) + (RecencyWeight) + (NetworkDensityWeight)
    const scoredPosts = posts.map(post => {
      let score = 0;
      
      // 1. Engagement (likes, reVibes, comments)
      if (post.stats) {
        score += (post.stats.likes || 0) * 2;
        score += (post.stats.reVibes || 0) * 3;
        score += (post.stats.comments || 0) * 1;
      }

      // 2. Recency (Time Decay)
      const hoursSince = (Date.now() - (post.timestamp || Date.now())) / 3600000;
      score += 100 / (hoursSince + 1);

      // 3. Network Density (Follower status)
      if (currentUser.following.includes(post.authorId)) {
        score += 50;
      }

      // 4. Relevance (Occupation match)
      const author = users.find(u => u.id === post.authorId);
      if (author && author.occupation === currentUser.occupation) {
        score += 30;
      }
    ...
    filteredFeed.sort((a, b) => b.feedScore - a.feedScore);
  ...
  app.get('/api/conversations', (req, res) => {
    ...
    privateMessages.forEach(m => {
      if (m.from === currentUser.id) participants.add(m.to);
      if (m.to === currentUser.id) participants.add(m.from);
    });
    
    const conversationList = Array.from(participants).map(pId => {
      const user = users.find(u => u.id === pId);
      const lastMsg = privateMessages.filter(m => 
        (m.from === currentUser.id && m.to === pId) ||
        (m.from === pId && m.to === currentUser.id)
      ).pop();
      return {
        user,
        lastMessage: lastMsg
  ...
  app.post('/api/posts', async (req, res) => {
    ...
    const { type, content, title, category, image, isUncensored, tag, tags } = req.body;
  ...
  // --- WebSocket Server for Lounge & Private Messaging ---
  ...
  wss.on('connection', (ws, req) => {
    ...
    if (loungeId) {
      ...
      const clientData = { ws, username, id: userId || Math.random().toString(36).substring(7) };
      room.add(clientData);
        ...
        try {
          ...
          if (parsed.type === 'chat') {
            ...
            if (targetWs && targetWs.readyState === WebSocket.OPEN) {
              targetWs.send(JSON.stringify({ type: 'private-chat', message: msg }));
            }
      ...
      // Pure private messaging connection
      ws.on('message', (data) => {
        try {
          const parsed = JSON.parse(data.toString());
          if (parsed.type === 'private-chat') {
            const { to, content } = parsed;
            const msg = {
              id: Math.random().toString(36).substring(7),
              from: userId,
